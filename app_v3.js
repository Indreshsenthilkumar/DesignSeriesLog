...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 🔄 Real-time Status Sync Polling (Every 5 seconds)
    setInterval(() => {
...
async function fetchAttendance(email) {
    ...
    try {
        ...
        if (data.status === "success" && data.student) {
            ...
            // 🛡️ SECURITY CHECK: Remote Logout or Block
...
async function populateDashboard(freshStudentData) {
    ...
    if (dateElModal) dateElModal.innerHTML = `<span>${todayStr}</span>`;
...
function renderHistory() {
    ...
    if (mobileDashboardList) {
        mobileDashboardList.innerHTML = `
            ...
            ${historyItems.map(i => `
                <div class="card" style="padding: 0.85rem 1rem; margin-bottom: 0.75rem; border: 1px solid #f1f5f9; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
