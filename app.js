...
window.handleLogout = async () => { 
    ...
    if (user && user.email) {
        ...
        } catch (e) { console.warn("Logout status sync failed"); }
    }
    localStorage.clear(); 
...
window.handleCredentialResponse = async function(response) {
    ...
    try {
        ...
        if (data.status === "success" && data.student) {
            ...
            // Mark as Logged_In in the sheet
            fetch(API_URL, { 
                method: 'POST', 
                body: JSON.stringify({ action: "updateStatus", email: payload.email, status: "Logged_In" }) 
            });
...
async function fetchAttendance(email) {
    ...
    try {
        ...
        if (data.status === "success" && data.student) {
            ...
            // 🛡️ SECURITY CHECK: Logout if admin has changed status to Blocked or Logged_Out
            if (status === "blocked") {
                localStorage.clear();
            ...
            if (status === "logged_out") {
                localStorage.clear();
                window.location.replace('login.html?logout=remote');
                return;
            }
...
function renderHistory() {
    ...
    const generateHourBubbles = (hours) => {
        return `<div style="display: flex; gap: 6px; justify-content: center;">
            ${hours.map(h => `<span style="width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; background: #eff6ff; color: #2563EB; border-radius: 50%; font-size: 0.85rem; font-weight: 900; border: 1.5px solid #93c5fd;">${h}</span>`).join('')}
        </div>`;
    };
    ...
    if (mobileHistoryList) {
        mobileHistoryList.innerHTML = `
            ...
            ${historyItems.map(i => `
                <div class="card" style="padding: 0.85rem 1rem; margin-bottom: 0.75rem; border: 1px solid rgba(0,0,0,0.06); box-shadow: 0 20px 40px rgba(0,0,0,0.08);">
                    ...
                    <div style="display: flex; gap: 8px;">
                        ${i.hours.map(h => `<span style="width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; background: #eff6ff; color: #2563EB; border-radius: 50%; font-size: 0.9rem; font-weight: 900; border: 1.5px solid #93c5fd;">${h}</span>`).join('')}
                    </div>
                </div>
            `).join('')}
        `;
        ...
        mobileDashboardList.innerHTML = `
            ...
            ${historyItems.map(i => `
                <div class="card" style="padding: 0.85rem 1rem; margin-bottom: 0.75rem; border: 1px solid rgba(0,0,0,0.06); box-shadow: 0 20px 40px rgba(0,0,0,0.08);">
                    ...
                    <div style="display: flex; gap: 8px;">
                        ${i.hours.map(h => `<span style="width: 26px; height: 26px; display: flex; align-items: center; justify-content: center; background: #eff6ff; color: #2563EB; border-radius: 50%; font-size: 0.85rem; font-weight: 900; border: 1.5px solid #93c5fd;">${h}</span>`).join('')}
                    </div>
                </div>
            `).join('')}
        `;
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        dayCell.onclick = () => {
            ...
            if (groupedData[dateStr]) {
                ...
                const hourBubbles = log.hours.map(h => `<span style="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; background: #eff6ff; color: #2563EB; border-radius: 50%; font-size: 0.95rem; font-weight: 900; border: 1.5px solid #93c5fd;">${h}</span>`).join('');
